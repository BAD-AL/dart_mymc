...
// ---------------------------------------------------------------------------
//
// CBS support has been removed because:
//   1. The format requires ZLibDecoder which is only available in dart:io,
//      blocking web (Wasm/JS) compatibility of the public API.
//   2. CodeBreaker (.cbs) files are extremely rare in practice and no test
//      fixtures are available to validate the implementation.
//
// To restore CBS support on native platforms, move this section back and
// add `import 'dart:io' show ZLibDecoder;` at the top of the file.
//
...
extension Ps2SaveFileFormats on Ps2SaveFile {
  ...
  // CBS load/save are not supported — see the RC4 section above for the reason.
  // Magic bytes are still detected so callers receive a clear UnsupportedError
  // rather than a generic "unrecognised format" message.
  void loadCbs(SaveIo f) {
    throw UnsupportedError(
      'CodeBreaker (.cbs) format is not supported: the decoder requires '
      'ZLibDecoder from dart:io, which is unavailable on web platforms. '
      'Convert the file to .psu first using a desktop tool.',
    );
  }
